...
class TablaReclamos
{
  ...
  public function mostrarTablaReclamos()
  {
    ...
    for ($i = 0; $i < count($reclamos); $i++) {
      ...
      } else if ($reclamos[$i]["estado"] == 1) {

        $botones = "<div class='btn-group'>";
        $botones .= "<button class='btn btn-default btnVerReclamo' title='ver' idReclamo='" . $reclamos[$i]["codigo"] . "' data-toggle='modal' data-target='#modalVerReclamo'><i class='fa fa-eye'></i></button>";
        $botones .= "<button class='btn btn-warning btnAsigReclamo' title='asignar' estado='" . $reclamos[$i]["estado"] . "' idReclamo='" . $reclamos[$i]["codigo"] . "' data-toggle='modal' data-target='#modalAsginarReclamo'><i class='fa fa-user'></i></button></div>";
      } else {
...
=============================================*/
$activarTablaReclamos = new TablaReclamos();
$activarTablaReclamos->mostrarTablaReclamos();
